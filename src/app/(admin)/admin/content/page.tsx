...
export default function AdminContentPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Sei sicuro di voler eliminare ${selectedIds.length} avvisi? Questa azione non può essere annullata.`)) {
      return;
    }
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Gestione Avvisi</h1>
          <p className="text-gray-500 mt-1 text-sm sm:text-base">Modifica gli avvisi della dashboard utente</p>
        </div>
        <Link
      ...
      {selectedIds.length > 0 && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="text-indigo-800 font-medium">{selectedIds.length} avvisi selezionati</span>
      ...
      {isLoading && (
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-gray-600">Caricamento avvisi...</p>
        </div>
      )}
