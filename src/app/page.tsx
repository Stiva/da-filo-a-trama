...
export default async function HomePage() {
  ...
  const logoFull = resolveBrandAssetUrl(bundle.brand.assets.logo_full) || '/Logo completo.png';
  const isRemote = logoFull.startsWith('http');
        ...
        <div className="text-center max-w-2xl w-full flex flex-col items-center">
          ...
          <div className="bg-white/80 backdrop-blur-md p-8 rounded-3xl shadow-sm border border-black/5 mb-12 text-gray-700 w-full">
            <p className="text-lg md:text-xl font-medium mb-3">
              {t('landing.card.headline.prefix')}{' '}
              <b className="text-agesci-blue">{t('landing.card.headline.highlight')}</b>{' '}
              {t('landing.card.headline.suffix')}
            </p>
        ...
        <div className="container-scout mx-auto px-6 space-y-8 flex flex-col">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-white/60">
            <div className="text-center md:text-left">
              <p className="font-semibold text-white/80">{t('landing.copyright')}</p>
            </div>
            <div className="text-center md:text-right flex flex-col items-center md:items-end gap-2">
              <p>{t('landing.branca')}</p>
              <div className="flex items-center gap-4 mt-2">
                <a href={t('landing.url.site')} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors" title="Sito Istituzionale dell'evento">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>
                </a>
                <a href={t('landing.url.instagram')} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors" title="Instagram Ufficiale">
