...
export const createIcon = (tipo: PoiCategory) => {
  const colors: Record<PoiCategory | string, string> = {
...
export const getTypeEmoji = (tipo: PoiCategory | string) => {
...
export default function MapContainer({ pois, selectedPoi, onPoiSelect, children, editableLayerRef, onMapReady }: MapProps) {
  ...
  // Pass map instance to parent if required
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      >
        ...
        {/* Poligoni (Aree) e Marker standard mischiati o separati */}
        <AreaLayer pois={pois} onPoiSelect={onPoiSelect} />

        {pois.map((poi) => {
          // I point standard hanno lat/long. Le aree potrebbero non avere un marker ma solo l'area, ma generiamo il marker in centro anche per le aree o li sdoppiamo.
          if (!poi.latitude || !poi.longitude) return null;
          // Se è un'area mostriamo il marker solo in edit mode? L'utente richiede che "Nessuno strumento di disegno" e "Click/tap su un'area apre popup".
          // Per le aree il popup lo gestisce AreaLayer. Evitiamo doppi popup.
          if (poi.area_polygon) return null;
            ...
            >
              <Popup>
                <div className="min-w-[150px]">
                  ...
                  <p className="text-sm text-gray-500">{POI_TYPE_LABELS[poi.tipo] || poi.tipo}</p>
                  {poi.descrizione && (
        ...
        {/* Layer gestito esternamente per admin drawing tools */}
        {children}
