...
export default function UserDropdownMenu() {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'Escape') {
                setIsOpen(false);
                const trigger = menuRef.current?.querySelector('button[aria-haspopup="menu"]') as HTMLButtonElement;
                trigger?.focus();
            }
        };
        if (isOpen) {
    ...
    return (
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div role="menu" className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-agesci-blue/10 overflow-hidden z-50">
